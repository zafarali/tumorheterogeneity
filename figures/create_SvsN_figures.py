...
ax.semilogx(np.arange(0,len(S),1)[S>0], S[S>0], '--b', label='>0%')
S = np.load('./turnover_close_001.npy')
ax.semilogx(np.arange(0,len(S),1)[S>0], S[S>0], '--r', label='>1%')
S = np.load('./turnover_close_01.npy')
ax.semilogx(np.arange(0,len(S),1)[S>0], S[S>0], '--g', label='>10%')
